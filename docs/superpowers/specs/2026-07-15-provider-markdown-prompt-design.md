...
- 模型直接输出自由 Markdown；只有题目规定的风险场景要求四个可见板块。
...
真实模型直接生成 Markdown 文本。SSE 增量立即追加到助手消息，并由 Markdown 组件渲染；前后端不增加 JSON 信封，也不解析中文板块名称。
...
考题明确要求“发布风险梳理”场景包含“结论、风险项、待确认项、下一步”，但没有规定所有普通回答都使用固定 JSON Schema。TaskBuddy 使用 Markdown 作为可读输出契约：模型自由组织普通回答；风险场景通过 Prompt 要求四个板块；模糊请求与拒绝场景使用自然语言。自动化或人工测试检查关键行为，失败时改进 Prompt，不在运行时解析特定中文字符串。
...
- 提示词/密钥窃取请求及其变体都拒绝泄露；
- 普通问题不会被强制生成无意义的风险项；
- 模型回复作为 Markdown 展示，不出现或解析内部 JSON 信封。
